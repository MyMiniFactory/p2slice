...
class Tweak:
    ...
    def target_function(self, bottom, overhang):
        ...
        if bottom == 0:
            ...
            Unprintability = 100/bottom * max(overhang, 0.01)
    ...
    def lithograph(self, mesh, orientation):
        ...
        if len(overhangs) > 0:
            overhang = np.sum(overhangs[:,7,0] * -np.inner(overhangs[:,0,:], orientation) * ((4/3) *  overhangs[:,7,2] - (1/6) * ( overhangs[:,7,2] + np.min(overhangs[:,6,:], axis=1))- (np.zeros(len(overhangs))+zmin)))
            overhang = overhang / volume * 100
        else: overhang = 0
        """

        #compute overhang with surface
        if len(overhangs) > 0:
            #  overhang = np.sum(overhangs[:,7,0] * 2
